...
//Creating Vectors
	...
	//std::unique_ptr<double> VectorMuon_Pt;  //Dont Compile
	//std::shared_ptr<double> VectorMuon_Pt;
...
JpsiAnalyzerOpen2011::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  lumiBlock = iEvent.luminosityBlock();
	...
	std::cout<<" ================================================================================ "<<std::endl;
	std::cout<<" Run, Evt, lumiBlock "<<std::endl;
	std::cout<< irun <<" "<< ievt <<" "<< lumiBlock<<std::endl;
	std::cout<<" ================================================================================ "<<std::endl;
  ...
  for (reco::MuonCollection::const_iterator muon = recoMuons->begin(); muon != recoMuons->end(); muon++) {
		...
		if ( (muon->pt() >= 7.0)  &&  ( muon->eta()  < 2.4  )  )  //pt trigger  
		{
			PtTrigger7++;
		...
		if (isTMOneStationTight) 
		{
			...
			std::cout<< " dxy: "<< fabs(muon->innerTrack()->dxy(vertex->position()))  << std::endl; 
  			std::cout<< " dz: "<< fabs(muon->innerTrack()->dz(vertex->position()))  << std::endl;
...
std::sort(myLeptons.begin(),myLeptons.end(), [](const reco::Muon &a, const reco::Muon &b){
  ...
  if (myLeptons.size() >= 2) { //loop ndimuon
	...
	VectorEvent.push_back(ievt);
	VectorRun.push_back(irun);
	VectorlumiBlock.push_back(lumiBlock);
...
JpsiAnalyzerOpen2011::beginJob()
{
	...
	AnalysisTree->Branch("PtTrigger7", &PtTrigger7, "PtTrigger7/I");
...
JpsiAnalyzerOpen2011::endJob()
{
	...
	std::cout << "CounterMuons: " << CounterMuons << std::endl; 
	std::cout << "PtTrigger7: " << PtTrigger7 << std::endl;
