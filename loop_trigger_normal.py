    ...
    if data_type == 'data' or data_type == 'pythia':
        if data_type == 'pythia':
            ...
            while j < NumberLinesData:                          #loop data.txt
                ...
                with open('JPSIAnaOpenData2011_Model_Pythia.py') as f: 
                    NumberLines = sum(1 for _ in f)             #number of lines of a model file
                fileModel = open('JPSIAnaOpenData2011_Model_Pythia.py')#Open the model file 
                ...
                while i < NumberLines:                          
                    ...
                    if i == 11 or i== 17:
                        if i == 11:
                            ...
                            i = i+1
                        if i == 17:
                            fileOutput.write("    'histo%s.root'" % (k) )
                            i = i+1
                    else :
                ...
                j = j + 1
                os.system("cmsRun JPSIAnaOpenData2011.py > log%s.log" % (k) ) #Command cmsRun
                print '------------histo%s.root done------------' % (k)
                ...
                shutil.move('log%s.log' % (k),'Output_Pythia')    #move file to a folder
                shutil.move('histo%s.root' % (k),'Output_Pythia')
                #End write file
    ...
    #--------------------------------------------------------        
        if data_type == 'data':
            ...
            while j < NumberLinesData:                          #loop data.txt
                ...
                shutil.move('log%s.log' % (k),'Output_trigger')    #move file to a folder
                shutil.move('histo%s.root' % (k),'Output_trigger')
                #End write file
